-- Ultra Type-Safe Friendship System Integration
-- Handles all possible UUID/TEXT conversion issues
...
-- 3. Ultra-safe function to get current user ID
...
DECLARE
  ...
  user_id_uuid UUID;
BEGIN
  -- Get auth.uid() as text first
  SELECT auth.uid()::TEXT INTO user_id_text;
...
-- 4. Function to create 1:1 chat room between friends
...
DECLARE
  ...
  larger_id UUID;
BEGIN
  -- Get current user using ultra-safe function
  SELECT get_current_user_uuid() INTO current_user_id;
...
-- 5. Update activity feed function to use friendships
...
-- 6. Add friendship-related policies to existing chat system
...
-- Create new policies using ultra-safe function
CREATE POLICY "Friends can create direct chats" ON chat_rooms
  FOR INSERT WITH CHECK (
    (type != 'direct') OR 
    (type = 'direct' AND get_current_user_uuid() = created_by)
  );

-- Enhanced policy for viewing chat rooms using ultra-safe function
CREATE POLICY "Users can view accessible chat rooms" ON chat_rooms
  FOR SELECT USING (
    -- Public rooms
    (NOT is_private) OR
    -- Rooms user created
    (created_by = get_current_user_uuid()) OR
    ...
    (
      ...
      EXISTS(
        ...
        AND (
          owner_id = get_current_user_uuid() OR 
          get_current_user_uuid() = ANY(tagged_users) OR
          privacy = 'public' OR
          (privacy = 'contacts' AND are_users_friends(get_current_user_uuid(), owner_id))
        )
      )
    )
  );
...
-- Success message
SELECT 'Ultra-safe friendship system integrated! 🎉' AS result;