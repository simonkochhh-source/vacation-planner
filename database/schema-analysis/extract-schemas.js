...
async function extractSchema(supabase, dbName) {
  ...
  try {
    ...
    console.log('    ℹ️  Using direct queries to check table accessibility...');
...
async function main() {
  ...
  // Compare tables
  const prodTables = Object.keys(results.production.tables);
  const devTables = Object.keys(results.dev.tables);

  comparison.differences.missing_tables_in_dev = prodTables.filter(t => 
    !devTables.includes(t) || !results.dev.tables[t].exists
  );
  
  comparison.differences.extra_tables_in_dev = devTables.filter(t => 
    !prodTables.includes(t) && results.dev.tables[t].exists
  );

  // Compare table structures
  for (const table of prodTables) {
    if (devTables.includes(table) && results.production.tables[table].exists && results.dev.tables[table].exists) {
