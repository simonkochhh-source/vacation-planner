-- Friendship System Integration with Existing Chat System (Type-Safe Version)
...
-- 3. Helper function to safely get current user ID as UUID
CREATE OR REPLACE FUNCTION get_current_user_id()
RETURNS UUID AS $$
BEGIN
  RETURN COALESCE((auth.uid())::UUID, NULL);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- Create new policies using helper function for type safety
CREATE POLICY "Friends can create direct chats" ON chat_rooms
  FOR INSERT WITH CHECK (
    (type != 'direct') OR 
    (type = 'direct' AND get_current_user_id() = created_by)
  );

-- Enhanced policy for viewing chat rooms using helper function
    ...
    (
      ...
      EXISTS(
        ...
        AND (
          ...
          get_current_user_id() = ANY(tagged_users) OR
          privacy = 'public' OR
          (privacy = 'contacts' AND are_users_friends(get_current_user_id(), owner_id))
        )
      )
    )
  );
