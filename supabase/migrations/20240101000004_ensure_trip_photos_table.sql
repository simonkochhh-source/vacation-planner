-- Migration: Ensure trip_photos table exists with correct schema
-- Description: Creates or updates trip_photos table structure

-- Drop existing table if it exists (to ensure clean state)
DROP TABLE IF EXISTS trip_photos CASCADE;

-- Create trip_photos table with all required fields
...
FOR SELECT USING (
    ...
    OR EXISTS (
        ...
        AND (t.privacy = 'public' OR t.owner_id = auth.uid() OR auth.uid() = ANY(t.tagged_users))
    )
);
